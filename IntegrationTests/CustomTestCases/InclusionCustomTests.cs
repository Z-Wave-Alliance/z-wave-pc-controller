...
namespace IntegrationTests
{
    ...
    class InclusionCustomTests : TestCaseBase
    {
        public InclusionCustomTests(ApiTypeUnderTest type, SecurityUnderTest SecurityUnderTestMain)
        ...
        public void AddNodeToSecondSIS_AfterMainUpdate_MainNodeListChanges(SecurityUnderTest secondaryControllerSecurity)
        {
            ...
            Assert.AreNotEqual(_MainVMPrimary.Nodes.Count, _MainVMSecondary.Nodes.Count);
            var expected = _MainVMSecondary.Nodes.Where(x => x.Item.EndPointId == 0).Count();
            ControllerUpdate();
        ...
        public void AddNodeToPrimary_WhenSecondarySIS_AutoUpdatesNodeList(SecurityUnderTest secondaryControllerSecurity)
        {
            ...
            Delay(500);
            var actual = _MainVMSecondary.Nodes.Where(x => x.Item.EndPointId == 0).Count();
            //Assert
            Assert.AreEqual(expected, actual);
            Assert_Node_IsSecured(_MainVMPrimary, 3);
            Assert_Node_IsSecured(_MainVMSecondary, 3);
        }
        ...
        public void SecondSISNoInclusionCC_InclusionAddNode_NodeAddedWithNeededScheme(SecurityUnderTest secondaryControllerSecurity)
        {
            ...
            RemoveInclusionSupportCC(_MainVMSecondary);
            AddNode(_MainVMPrimary, _MainVMSecondary);
            SetSelectedNode(0x02);
            SetAsSIS();
            AddNode(_MainVMSecondary);
